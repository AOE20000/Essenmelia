...
为了保护用户隐私，系统对所有 API 调用进行权限审计：
- **受限模式**：对于处于“受限访问”状态的扩展，部分敏感 API 的调用结果可能不代表真实系统状态。扩展应始终处理可能的权限受限情况。
- **动态权限**：用户可能随时更改权限授予情况，扩展应通过 `onPermissionGranted` 回调及时响应状态变更。
- **静默策略**：在受限模式下，系统可能会根据隐私策略自动处理某些请求，而不一定会产生可见的错误。
