...
- `README.md`: 包含 HTML 元数据注释（见下文），以便应用发现仓库。
...
**关键点：**
- 必须包含 `ESSENMELIA_METADATA` 关键字。
- 必须是有效的 JSON 格式。
- 建议为仓库添加 `essenmelia-extension` 话题（Topic）以增加曝光。
...
- **如何更新扩展？**: 只需修改仓库中的文件并增加 `manifest.yaml` 中的 `version` 字段。应用会提示用户有新版本可用。
