...
/// 默认扩展：外部调用入口
/// 职责：作为外部 Intent、ADB 或第三方应用调用的统一入口点和审计层
class ExternalCallExtension extends BaseExtension {
  static const String extensionId = 'system.external_call';

  ExternalCallExtension()
    : super(
        const ExtensionMetadata(
          id: extensionId,
          name: '外部调用入口',
          description: '处理来自 ADB、Intent 或第三方应用的 API 调用请求。',
          author: 'System',
          version: '1.0.0',
          icon: Icons.api,
          requiredPermissions: [
            ExtensionPermission.readEvents,
            ExtensionPermission.writeEvents,
            ExtensionPermission.readTags,
  ...
  Future<void> onInit(ExtensionApi api) async {
    _api = api;
    debugPrint('ExternalCallExtension initialized');
  }
  ...
  Future<dynamic> handleExternalRequest(
    ...
    debugPrint('ExternalCallExtension: Handling external request [$method]');
    dynamic result;
    bool success = true;
    String? error;
    ...
    try {
      switch (method) {
        ...
        case 'addEvent':
          ...
          break;
        case 'showSnackBar':
          _api.showSnackBar(params['message'] ?? '');
          result = true;
          break;
        case 'addStep':
          final eventId = params['eventId'];
          final description = params['description'];
          if (eventId != null && description != null) {
            await _api.addStep(eventId, description);
            result = true;
          } else {
            result = false;
          }
...
class _ExternalCallPage extends StatefulWidget {
  final ExternalCallExtension extension;
            ...
            SliverPadding(
              ...
              sliver: SliverAnimatedList(
                ...
                itemBuilder: (context, index, animation) {
                  ...
                  return widget.extension._buildLogItem(
                    context,
                    log,
                    animation,
                  );
